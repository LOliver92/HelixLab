function newSwimmingActivity(rd){
    alert(rd.msg);
    if(rd.result == "1"){
...
function sendRecive(postFile, dataInput, processName){
    $.ajax({
        ...
        success:function(rd){
            //alert(rd);
            if(processName == "newSwimmingActivity") { newSwimmingActivity(rd); }
        },
        error:function(){
...
function recordNewAct(){
    ...
    var data = {'task':'newSwimmingActivity', 'numLongOfActKm' : numLongOfActKm.value, 'numLongOfActM' : numLongOfActM.value,
    ...
    if(numLongOfActKm.value != "" && numLongOfActM.value != "" && timeOfAct.value != "" &&  dateOfAct.value != ""){
        sendRecive("../Controller/ActivityController.php", data, "newSwimmingActivity");
    }
    else{
